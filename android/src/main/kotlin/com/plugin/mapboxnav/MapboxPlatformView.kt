...
class MapboxPlatformView(
    ...
    private val routesObserver = RoutesObserver { routeUpdateResult ->
        if (routeUpdateResult.navigationRoutes.isNotEmpty()) {
            ...
            //routeArrowApi.clearArrows { value ->
            //    _mapView?.mapboxMap?.style?.apply { routeArrowView.render(this, value) }
            //}
    ...
    private fun findRoute(destination: Point) {
        val originLocation = navigationLocationProvider.lastLocation ?: run {
            Log.e(TAG, "Localização atual não disponível para criar rota.")
            sendEvent("error", mapOf("message" to "Localização atual não disponível."))
            return
        }
            ...
            RouteOptions.builder()
                ...
                .apply {
                    originLocation.bearing?.let { bearing ->
                        bearingsList(
                            listOf(
                                Bearing.builder().angle(bearing).degrees(45.0).build(),
                                null
                            )
                        )
                    }
    ...
    private fun replayOriginLocation() {
        mapboxNavigation?.mapboxReplayer?.apply {
            play()
            pushEvents(
                listOf(
